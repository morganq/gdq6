...
class Map(gameobj.GameObject):
    def __init__(self, game, batch, width, height, x=0, y=0):
        ...
        for edge in self.edges:
            color = {
                'highway':(128,0,128,255),
                'main':(255,80,80,255),
                'basic':(128,128,128,255),
            }[edge.road.kind]
            dd = (edge.node1.pt - edge.node2.pt).get_normalized()
            self.lines.append((edge.node1.pt, edge.node2.pt, color))
    ...
    def _take_step(self):
        ...
        for border in self.borders:
            self.lines.append((border[0], border[1], (0,0,0,255)))


    def _draw_square(self, pt, color):
        SIZE = .5
        self.lines.append((pt + Vector(-SIZE, -SIZE), pt + Vector(SIZE, -SIZE), color))
        self.lines.append((pt + Vector(SIZE, -SIZE), pt + Vector(SIZE, SIZE), color))
        self.lines.append((pt + Vector(SIZE, SIZE), pt + Vector(-SIZE, SIZE), color))
        self.lines.append((pt + Vector(-SIZE, SIZE), pt + Vector(-SIZE, -SIZE), color))
        ...
        for border in self.borders:
            ...
            if p:
                if t <= 0:
                    ...
                    linepts[1] = p
        road = Road(linepts[0], linepts[1], direction, "highway")
    ...
    def try_add_road(self, point, direction, kind):
        ...
        if pos_pt and neg_pt and pos_pt != neg_pt:
            ...
            road = Road(neg_pt, pos_pt, direction, kind)
            self.roads.append(road)
