import gameobj
from vector import Vector
import pyglet
import random
from maphelp import *
import math

class PolygonGroup(pyglet.graphics.Group):
    def __init__(self, obj):
        self.obj = obj
        super().__init__()

    def set_state(self):
        pyglet.gl.glPushMatrix()
        pyglet.gl.glLoadIdentity()
        pyglet.gl.glTranslatef(self.obj.x, self.obj.y, 0)

    def unset_state(self):
        pyglet.gl.glPopMatrix()

class MapNode:
    def __init__(self, x, y):
        self.x = x
        self.y = y
        self.edges = []

    def add_edge(self, edge):
        self.edges.append(edge)

class Road:
    def __init__(self, a, b, direction, kind):
        self.a = a
        self.b = b
        self.direction = direction
        self.kind = kind

class MapGenerator(gameobj.GameObject):
    def __init__(self, game, batch, width, height, x=0, y=0):
        self.batch = batch
        self.width = width
        self.height = height
        self.nodes = []

        self.borders = []
        self.lines = []
        self.roads = []

        self._group = PolygonGroup(self)
        self._vertex_list = None
        self._steps = [
            self.gen_border,
            self.gen_highway,
        ]
        for i in range(random.randint(2,7)):
            self._steps.append(self.gen_main_road)

        for i in range(200):
            self._steps.append(self.gen_basic_road)

        self._step_timer = 0.25
        
        super().__init__(game, x, y)

    def _take_step(self):
        step = self._steps.pop(0)
        step()
        self._update_vertex_list()

    def _update_vertex_list(self):
        num = len(self.lines) * 2
        verts = []
        colors = []
        for a,b,color in self.lines:
            verts.extend([a.x,a.y,b.x,b.y])
            colors.extend(color)
            colors.extend(color)
        if not self._vertex_list:
            self._vertex_list = self.batch.add(
                num,
                pyglet.gl.GL_LINES,
                self._group,
                ('v2f', tuple(verts)),
                ('c4B', colors)
            )
        else:
            self._vertex_list.resize(num)
            self._vertex_list.vertices[:] = verts
            self._vertex_list.colors[:] = colors

    def update(self, delta_time):
        self._step_timer -= delta_time
        if self._step_timer <= 0 and self._steps:
            self._step_timer += 0.0
            self._take_step()

    def gen_border(self):
        self.borders=[
            (Vector(0,0), Vector(self.width, 0)),
            (Vector(self.width,0), Vector(self.width, self.height)),
            (Vector(self.width,self.height), Vector(0, self.height)),
            (Vector(0,self.height), Vector(0, 0))
        ]
        self.lines.append((Vector(0,0), Vector(self.width, 0), (255,255,255,255)))
        self.lines.append((Vector(self.width,0), Vector(self.width, self.height), (255,255,255,255)))
        self.lines.append((Vector(self.width,self.height), Vector(0, self.height), (255,255,255,255)))
        self.lines.append((Vector(0,self.height), Vector(0, 0), (255,255,255,255)))

    def gen_highway(self):
        central_point = Vector(
            random.random() * self.width / 2 + self.width / 4,
            random.random() * self.height / 2 + self.height / 4,
        )
        direction = Vector.from_circular(random.random() * 3.1415, 1)
        linepts = [None, None]
        for border in self.borders:
            p, t = intersect(central_point, direction, border[0], border[1])
            if p:
                if t <= 0:
                    linepts[0] = p
                else:
                    linepts[1] = p
        self.lines.append((*linepts, (255,0,0,255)))
        self.roads.append(Road(linepts[0], linepts[1], direction, "main"))

    def gen_main_road(self):
        TOO_CLOSE = 100
        for i in range(5):
            d = 0
            attempts = 0
            point = None
            while d < TOO_CLOSE ** 2 and attempts < 100:
                point = Vector(
                    random.random() * self.width,
                    random.random() * self.height
                )
                mains = filter(lambda x:x.kind == "main", self.roads)
                for main in mains:
                    d = dist_sq_from_line(point, main.a, main.b)

                attempts += 1

            direction = Vector.from_circular(random.random() * 3.1415, 1)
            success = self.try_add_road(point, direction, "main")
            if success:
                return

    def gen_basic_road(self):
        TOO_CLOSE = 15
        closest_road_direction = None
        d = 0
        attempts = 0
        point = None
        while d < TOO_CLOSE ** 2 and attempts < 100:
            d = 999999999
            point = Vector(
                random.random() * self.width,
                random.random() * self.height
            )
            mains = filter(lambda x:x.kind == "main", self.roads)
            for main in mains:
                cd = dist_sq_from_line(point, main.a, main.b)
                if cd < d:
                    d = cd
                    closest_road_direction = main.direction

            attempts += 1

        if random.random() > 0.5:
            closest_road_direction = Vector(-closest_road_direction.y, closest_road_direction.x)
        success = self.try_add_road(point, closest_road_direction, "basic")
        if success:
            return      

    def try_add_road(self, point, direction, kind):
        MIN_DIST = 30
        intersecting_roads = []
        intersectable = [(rd.a, rd.b, rd) for rd in self.roads]
        intersectable.extend([(a,b,None) for a,b in self.borders])
        for a,b,r in intersectable:
            p, t = intersect(point, direction, a,b)
            if p:
                intersecting_roads.append((p,t,r))

        pos_pt = None
        neg_pt = None

        roads_pos = sorted(filter(lambda x:x[1]>=0, intersecting_roads), key=lambda x:x[1])
        roads_neg = sorted(filter(lambda x:x[1] <0, intersecting_roads), key=lambda x:-x[1])

        if kind == "basic":
            if random.random() > 0.66:
                roads_pos = roads_pos[0:1]
            if random.random() > 0.66:
                roads_neg = roads_neg[0:1]

        for p, t, r in roads_pos:
            closest = 999999999
            for road in self.roads:
                if road == r:
                    continue                
                dist = dist_sq_from_line(p, road.a, road.b)                
                closest = min(closest, dist)

            if closest > MIN_DIST ** 2:
                pos_pt = p
                if neg_pt is None:
                    neg_pt = p
            else:
                break

        for p, t,r  in roads_neg:
            closest = 999999999
            for road in self.roads:
                if road == r:
                    continue                
                dist = dist_sq_from_line(p, road.a, road.b)
                closest = min(closest, dist)

            if closest > MIN_DIST ** 2:
                neg_pt = p
                if pos_pt is None:
                    pos_pt = p
            else:
                break                    

        if pos_pt and neg_pt and pos_pt != neg_pt:
            c = {
                'main': (255,128,128,255),
                'basic': (128,128,128,255),
            }
            self.lines.append((neg_pt, pos_pt, c[kind]))
            self.roads.append(Road(neg_pt, pos_pt, direction, kind))
            return True
        else:
            return False